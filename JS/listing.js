...
$(addBtn).click(() => {
  if (addBtn.classList.contains('rotate')) {
    $(addBtn).removeClass('rotate')
    $('.input-type').hide()
  }
  else {
    $(addBtn).addClass('rotate')
    $('.input-type').show()
  }
})
...
function createTasks(div, obj) {
  ...
  const nameDateDiv = document.querySelectorAll(".nameDateDiv")
  $("#default-search").on("keyup", (e) => {
    let searchValue = e.target.value.toLowerCase().trim();
    for (let i = 0; i < content.length; i++) {
      console.log(nameDateDiv[i]);

      if (content[i].innerText.toLowerCase().indexOf(searchValue) != -1) {
        content[i].style.display = "block"
      } else {
        content[i].style.display = "none"
        // nameDateDiv[i].style.backgroundColor = "none"
      }
    }
  })
  ...
  showMoreBtn[i].addEventListener("click", () => {
    ...
    switch (showMoreBtn[i].id) {
      case "1":
        ...
        break;
      case "2":
        model_title.innerHTML = "SCHEDULE A TIME TO DO"
        break;
      case "3":
        model_title.innerHTML = "WHO CAN DO IT FOR YOU"
        break;
      case "4":
        model_title.innerHTML = "ELIMMINATE IT"
        break;
      default:
        ...
        <div class="second-div">
          <div class="text-base leading-6 text-gray-900 no-underline model-title " id="modalBar" data-id="modal-title">
            <div class="add-Cmt" id="commentDiv">
              <div class="comment-inner" id="relativeDiv">
          ...
          <div class="text-base leading-6 text-gray-900 no-underline " id="modal-title">
            <div class="change">
              ...
              <div class="commentBtnDiv" id="${element.id}">
                <button class="addComentBtn" data-id="${element.id}" title="Add Comment">
                  <span class="material-symbols-outlined">
                    chat
                  </span>
                </button>
                <button class="commentAddeds" id="${element.id}"><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
$(document).mouseup(function () {
  if (User_detailsdiv.classList.contains("showuserdata")) {
    $(".black-screen").hide();
...
let profileView = document.querySelector("#getUserinfo");
